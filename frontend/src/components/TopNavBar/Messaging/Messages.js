...
const Messages = ({
  ...
  const [selectedChat, setSelectedChat, messengerList ] = useContext(messaging);
  
  const selectMessage = async (id) => {
    const selectedMessage = messengerList.find((prev) => prev.message_id === id);
    ...
    socket.emit('joinRoom', { UUID: selectedMessage.target_uuid });
    toggle();
    openChatbox();
  };
      ...
      <div className="flex flex-col m-2 text-xs md:text-sm lg:text-base">
        ...
        >
          <div className="flex justify-between items-center">
            ...
            <Tooltip content={connected ? 'Connected' : 'Disconnected'} animation="duration-500">
              <div className={`bg-${connected ? 'green' : 'red'}-600 rounded-full p-1 drop-shadow-sm`} />
            </Tooltip>
          </div>
          <button onClick={() => {createNewChat(); toggle();}}>
            <RiEdit2Fill
