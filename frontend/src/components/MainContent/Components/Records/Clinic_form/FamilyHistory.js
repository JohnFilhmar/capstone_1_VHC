...
const FamilyHistory = ({ selectedTheme, isVisible, setIsVisible }) => {
  const [visibility, setVisibility] = useState('health_history');
  const {familyHistory: medicalHistory, setFamilyHistory: setMedicalHistory} = useContext(formDataContext);
  ...
  useEffect(() => {
    const time = setTimeout(() => {
      ...
      const updatedClinicForm = {
        ...oldClinicForm,
        family_medical_history: medicalHistory
      };
    ...
    <div className={`flex flex-col gap-0 p-2 m-2 border-b-2 border-solid border-${selectedTheme}-500 drop-shadow-lg shadow-md rounded-lg lg:w-full`}>
      <div className={`flex gap-1 justify-between mb-2`}>
        <div className="flex gap-1">
          ...
          <p className={`text-${selectedTheme}-500 font-bold`}>Family Medical History</p>
        </div>
